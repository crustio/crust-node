...
teeLaunch()
{
    ...
    crontab_cmd_with_if="t_pid=$(ps -ef | grep "$cmd_run" | grep -v grep | awk '{print $2}'); if [x"\$t_pid" == x""] && [ -f $recover_file_path ]; then crust-client tee-launch $config_path -b $log_path; fi"
